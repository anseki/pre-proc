...
function replaceTag(tag, replacement, content, srcPath, pathTest) {
  ...
  var maxReplace = replacement.length - 1, replaceValue = '';
...
/**
 * @param {Array.<{tag: (string|string[]), replacement: (string|string[])}>} replacements - Objects containing tag and your respective value to replace .
 * @param {string} content - A content that is processed.
 * @param {string} [srcPath] - A full path to the source file.
 * @param {(string|RegExp|Array)} [pathTest] - The content is changed when any test passed.
 * @returns {string} - A content that might have been changed.
 */
function replaceTagsDistinctValues(replacements, content, srcPath, pathTest) {
  ...
  return replacements.reduce(function(content, replacement) {
	...
	return replaceTag(tag, replacement, content, srcPath, pathTest);
  }, content);
}
...
exports.replaceTagsDistinctValues = replaceTagsDistinctValues;
